...
import { useClaimAudit } from '../claimAudit'
...
const FIELD_LABELS: Record<string, string> = {
  ...
  amountDenied: 'Monto denegado',
  amountUnprocessed: 'Monto no procesado',
  deductibleApplied: 'Deducible aplicado',
  copayApplied: 'Copago aplicado',
  submittedDate: 'Fecha presentación',
  settlementDate: 'Fecha liquidación',
  settlementNumber: 'Número liquidación',
  settlementNotes: 'Notas liquidación',
  pendingReason: 'Razón pendiente',
  returnReason: 'Razón devolución',
  cancellationReason: 'Razón cancelación',
  policyId: 'Póliza',
  status: 'Estado',
}
...
function formatValue(value: unknown, fieldKey: string): string {
  ...
  if (
    ...
    fieldKey === 'deductibleApplied' ||
    fieldKey === 'copayApplied'
  ) {
  ...
  if (fieldKey === 'status' && typeof value === 'string') {
    return ClaimStatusLabel[value as ClaimStatus] ?? value
  }

  // CareType enum
  if (fieldKey === 'careType' && typeof value === 'string') {
    return CareTypeLabel[value as CareType] ?? value
  }
...
function getStatusTransition(changes: ChangesData | null): { from: ClaimStatus; to: ClaimStatus } | null {
  if (!changes?.before?.status || !changes?.after?.status) return null
  return {
    from: changes.before.status as ClaimStatus,
    to: changes.after.status as ClaimStatus,
  }
}
...
function buildNarrative(item: AuditLogItemDto, changes: ChangesData | null): NarrativeResult {
  switch (item.action) {
    case 'CREATE':
      return { action: 'creó el reclamo', details: null }
    ...
    <div className="group">
      ...
      >
        <p className="text-sm text-slate-600 leading-relaxed">
          ...
          {transition && (
            <StatusBadge
              label={ClaimStatusLabel[transition.to]}
              variant={getClaimStatusVariant(transition.to)}
...
export function ClaimHistoryTab({ claimId }: ClaimHistoryTabProps) {
  const [page, setPage] = useState(1)
  const { data, isLoading, isError, error } = useClaimAudit(claimId, { page, limit: 20 })
      ...
      <div className="rounded-xl bg-white shadow-sm ring-1 ring-slate-900/5 overflow-hidden">
        ...
        <div className="p-8 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Aún no hay actividad registrada para este reclamo
          </p>
        </div>
      </div>
    )
