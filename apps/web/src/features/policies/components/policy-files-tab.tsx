...
import type { PolicyFileCategory, PolicyFileDto } from '@claims/shared'
import {
  Button,
  FileDropzoneTrigger,
  FileCard,
...
// Policy-specific uploading file type
type PolicyUploadingFile = UploadingFile<PolicyFileCategory>

interface PolicyFilesTabProps {
  policyId: string
}

export function PolicyFilesTab({ policyId }: PolicyFilesTabProps) {
  const { data, isLoading, isError } = usePolicyFiles(policyId)
  const uploadMutation = useUploadPolicyFile(policyId)
  const deleteMutation = useDeletePolicyFile(policyId)

  // Local state for files being uploaded
  const [uploadingFiles, setUploadingFiles] = useState<PolicyUploadingFile[]>([])
  const [deletingFileId, setDeletingFileId] = useState<string | null>(null)
  const [deleteConfirm, setDeleteConfirm] = useState<PolicyFileDto | null>(null)
  ...
  const handleFilesSelected = useCallback((newFiles: File[]) => {
    const pending: PolicyUploadingFile[] = newFiles.map((file) => ({
  ...
  const handleCategoryChange = useCallback(
    (fileId: string, category: PolicyFileCategory | null) => {
      setUploadingFiles((prev) =>
...
function formatTotalSize(files: PolicyFileDto[]): string {
