...
function getClientStatusVariant(isActive: boolean): BadgeVariant {
  return isActive ? 'success' : 'default'
}

function getClientStatusLabel(isActive: boolean): string {
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 md:gap-4 pb-3 md:pb-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-1">
              ...
              />
              <StatusBadge
                label={getClientStatusLabel(isActive)}
                variant={getClientStatusVariant(isActive)}
              />
            </div>
          </div>
        </div>
