...
function getAffiliateStatusVariant(isActive: boolean): BadgeVariant {
  return isActive ? 'success' : 'default'
}

function getAffiliateStatusLabel(isActive: boolean): string {
  return isActive ? 'Activo' : 'Inactivo'
}

function getAffiliateTypeVariant(isOwner: boolean): BadgeVariant {
  return isOwner ? 'info' : 'warning'
}

function getAffiliateTypeLabel(isOwner: boolean): string {
  return isOwner ? 'Titular' : 'Dependiente'
}

/* -----------------------------------------------------------------------------
 * AffiliateDetailHeader Component
 * -------------------------------------------------------------------------- */

interface AffiliateDetailHeaderProps {
  firstName: string
  lastName: string
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-4 md:pt-6">
        ...
        <nav aria-label="Breadcrumb" className="mb-2 md:mb-3">
          ...
          >
            ...
            <span className="hidden sm:inline">Volver a Afiliados</span>
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 md:gap-4 pb-3 md:pb-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-1">
              ...
              <StatusBadge
                label={getAffiliateTypeLabel(isOwner)}
                variant={getAffiliateTypeVariant(isOwner)}
              />
              <StatusBadge
                label={getAffiliateStatusLabel(isActive)}
                variant={getAffiliateStatusVariant(isActive)}
              />
            </div>
          </div>
        </div>
