...
import { useClients, ClientFormSheet, type ClientsQueryParams } from '@/features/clients'
import type { ClientListItemDto, ClientsSortBy } from '@claims/shared'

// Sortable columns - defined first so it can be used in schema
const SORTABLE_COLUMNS = ['createdAt', 'name', 'taxId'] as const
...
const columnHelper = createColumnHelper<ClientListItemDto>()

function ClientsListPage() {
  const navigate = useNavigate()
  const searchParams = Route.useSearch()

  // New client form sheet state
  ...
  const updateSearch = useCallback(
    (updates: Partial<ClientsSearch>) => {
      void navigate({
        from: Route.fullPath,
        search: (prev: ClientsSearch) => ({ ...prev, ...updates }),
      })
    },
    [navigate]
  )
  ...
  const queryParams: ClientsQueryParams = useMemo(
    () => ({
      ...
      sortBy: searchParams.sortBy as ClientsSortBy,
  ...
  const columns: ColumnDef<ClientListItemDto, unknown>[] = useMemo(
    () => [
      columnHelper.accessor('name', {
        ...
        cell: ({ getValue, row }) => (
          <button
            type="button"
            onClick={() => void navigate({ to: '/clients/$clientId', params: { clientId: row.original.id } })}
      ...
      columnHelper.accessor('affiliateCount', {
        header: 'Afiliados',
        size: 100,
        cell: (info) => info.getValue(),
        enableSorting: false,
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 space-y-6 mb-6">
        <PageHeader
          ...
          actions={
            <Button onClick={() => setNewClientSheetOpen(true)}>
        ...
        {isError && (
          <Alert variant="error" title="Error al cargar clientes">
            <p>No se pudieron cargar los clientes. Por favor intente nuevamente.</p>
            <Button
              variant="outline"
              size="sm"
      ...
      <ClientFormSheet
        open={newClientSheetOpen}
        onOpenChange={setNewClientSheetOpen}
      />
      ...
      <div className="md:hidden">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-slate-500 font-mono">{client.taxId}</p>
                </div>
                <StatusBadge
              ...
              <div className="mt-3 flex items-center gap-4 text-sm text-slate-600">
                <span>{client.policyCount} pólizas</span>
                <span>{client.affiliateCount} afiliados</span>
              </div>
            </div>
          )}
        ...
        <CardListLoadMore
          ...
          isLoading={isLoading}
          hasMore={(data?.clients.length ?? 0) < (data?.meta.totalCount ?? 0)}
          currentCount={data?.clients.length ?? 0}
